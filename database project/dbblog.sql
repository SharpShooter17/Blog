...
-- Schema dbBlog
-- -----------------------------------------------------
CREATE SCHEMA IF NOT EXISTS `dbBlog` ;
USE `dbBlog` ;
...
-- -----------------------------------------------------
-- Table `dbBlog`.`session`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `dbBlog`.`session` ;

CREATE TABLE IF NOT EXISTS `dbBlog`.`session` (
  `session_id` INT NOT NULL AUTO_INCREMENT,
  `user_id` INT UNSIGNED NOT NULL,
  `token` VARCHAR(256) NOT NULL,
  `expiries` DATETIME NOT NULL,
...
START TRANSACTION;
USE `dbBlog`;
INSERT INTO `dbBlog`.`blog` (`blog_id`, `user_id`, `blog_category_id`, `name`, `last_modification`, `cretion_date`) VALUES (1, 1, 1, 'Mój pierwszy blog', '', '22-11-2017 16:32:00');
